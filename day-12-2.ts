...
function isAccessibleNode(node:TreeNode, path: string[], map: TreeNodeMap) {
  if(node.id === 'start') {
    return false;
  }

  let smallCaveVisitedTwice = false;
  Object.keys(map).filter(key => map[key].small).map(key => {
    if(path.filter(pathKey => pathKey === key).length > 1) {
      smallCaveVisitedTwice = true;
    }
  });
  
  return !smallCaveVisitedTwice || !(node.small && path.includes(node.id));
}

function findAllPaths(currentNode: TreeNode, currentPath: string[], map: TreeNodeMap) {
  const possibleNextNodes = currentNode.leafs.map(id => map[id]).filter(node => isAccessibleNode(node, currentPath, map));
